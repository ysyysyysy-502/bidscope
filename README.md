...
BidScope 是参加 2026 AI 先锋未来人才大赛超聚变企业命题的工程 Demo。项目未入围或获奖。用户输入一句自然语言需求，系统解析主题、地区、时间与频率，规划数据源，完成标准化、证据分层、相关性判断、去重和增量记录，最后返回结构化结果与 Word 报告。

> 当前仓库使用脱敏样例数据和模拟连接器，用于验证端到端流程；不代表已经接入所有真实外部平台。三人团队共同完成开题报告，产品方案落地及全部工程代码由核心开发者独立完成。
